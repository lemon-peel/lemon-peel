...
import { obtainAllFocusableElements } from '@lemon-peel/utils';
...
const TrapFocus: ObjectDirective = {
  beforeMount(element: TrapFocusElement) {
    element[FOCUSABLE_CHILDREN] = obtainAllFocusableElements(element);
    FOCUS_STACK.push(element);
    if (FOCUS_STACK.length <= 1) {
      document.addEventListener('keydown', FOCUS_HANDLER);
    }
  },
  updated(element: TrapFocusElement) {
    nextTick(() => {
      element[FOCUSABLE_CHILDREN] = obtainAllFocusableElements(element);
    });
  },
