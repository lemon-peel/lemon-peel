...
export function getFirstValidNode(
  ...
  return Array.isArray(nodes) ? getChildren(nodes[0], maxDepth) : getChildren(nodes, maxDepth);
}

export function renderBlock(...arguments_: Parameters<typeof createBlock>) {
  return openBlock(), createBlock(...arguments_);
}

export function renderIf(
  condition: boolean,
  ...arguments_: Parameters<typeof createBlock>
) {
  return condition ? renderBlock(...arguments_) : createCommentVNode('v-if', true);
}
...
export const getNormalizedProps = (node: VNode) => {
  ...
  for (const key of Object.keys(type)) {
    if (hasOwn(type[key], 'default')) {
      properties[key] = type[key].default;
    }
  }

  for (const key of Object.keys(raw)) {
    properties[camelize(key)] = raw[key];
  }
