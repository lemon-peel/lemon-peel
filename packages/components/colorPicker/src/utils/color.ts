...
const hsv2hsl = function (hue: number, sat: number, value: number) {
  return [
    hue,
    (sat * value) / ((hue = (2 - sat) * value) < 1 ? hue : 2 - hue) || 0,
    hue / 2,
  ];
};
...
const hsv2rgb = function (h: number, s: number, v: number) {
  ...
  const q = v * (1 - f * s);
  const t = v * (1 - (1 - f) * s);
  const module_ = index % 6;
  const r = [v, q, p, p, t, v][module_];
  const g = [t, v, v, q, p, p][module_];
  const b = [p, p, t, v, v, q][module_];
...
export default class Color {
  ...
  set(property: { [key: string]: any } | any, value?: number) {
    ...
    (this as any)[`_${property}`] = value;
    this.doOnChange();
  }

  get(property: string) {
    if (property === 'alpha') {
      return Math.floor(this[`_${property}`]);
    }
  ...
  protected fromHSV(h: number, s: number, v: number) {
  ...
  fromString(value: string) {
    ...
    if (value.includes('hsl')) {
      const parts = value
        ...
        .map((value_, index) =>
          index > 2 ? Number.parseFloat(value_) : Number.parseInt(value_, 10),
        );
      ...
      if (parts.length >= 3) {
        this.fromHSV(parts[0], parts[1], parts[2]);
      }
      ...
      if (!/^[\dA-Fa-f]{3}$|^[\dA-Fa-f]{6}$|^[\dA-Fa-f]{8}$/.test(hex))
        return;
