...
describe('Progress.vue', () => {
  test('percent', () => {
    ...
    expect(wrapper.find('.lp-progress__text').text()).toBe('66%');
    expect(wrapper.find('.lp-progress-bar__inner').attributes('style')).toBe(
  ...
  test('text inside', () => {
    ...
    expect(wrapper.classes()).toContain('lp-progress--text-inside');
  });

  test('stroke width', () => {
    const wrapper = mount(() => <Progress strokeWidth={7} />);

    expect(wrapper.find('.lp-progress-bar__outer').attributes('style')).toBe(
      'height: 7px;',
    );
  });
  ...
  test('show text', () => {
    ...
    expect(wrapper.find('.lp-progress__text').exists()).toBe(false);
  });

  test('circle', () => {
    const wrapper = mount(() => <Progress type="circle" />);

    expect(wrapper.classes()).toContain('lp-progress--circle');
  });

  test('dashboard', () => {
    const wrapper = mount(() => <Progress type="dashboard" />);

    expect(wrapper.classes()).toContain('lp-progress--dashboard');
  });

  test('width', () => {
    const wrapper = mount(() => <Progress type="circle" width={120} />);

    expect(wrapper.find('.lp-progress-circle').attributes('style')).toBe(
  ...
  test('color', () => {
    ...
    expect(
      wrapper.find('.lp-progress-bar__inner').attributes('style'),
    ...
    const wrapper = mount(() => (
      <Progress
        ...
        color={(percentage: number) => {
          return percentage > 50 ? 'rgb(4, 5, 6)' : 'rgb(1, 2, 3)';
        }}
      />
    ));
  ...
  test('format', () => {
    ...
    expect(wrapper.find('.lp-progress__text').text()).toBe('占比100%');
  });
  ...
  test('slot', () => {
    ...
    expect(wrapper.find('.lp-progress__text').text()).toBe('自定义内容');
  });
});
