...
| ((this: GlobalEventHandlers, event_: Event) => any)
...
const mouseMoveDocumentHandler = (e: MouseEvent) => {
  ...
  const previousPage = thumbState.value[bar.value.axis];
  ...
  const offset =
    (instance.value.getBoundingClientRect()[bar.value.direction] -
      e[bar.value.client]) *
    -1;
  const thumbClickPosition = thumb.value[bar.value.offset] - previousPage;
...
const clickThumbHandler = (e: MouseEvent) => {
  ...
  thumbState.value[bar.value.axis] =
    element[bar.value.offset] -
    (e[bar.value.client] - element.getBoundingClientRect()[bar.value.direction]);
};
