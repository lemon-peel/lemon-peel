...
import { getStyle } from '@lemon-peel/utils';
...
describe('Message on command', () => {
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(LpMessage._context).toBe(null);
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      LpMessage.install?.(testContext as any);
      expect(LpMessage._context).not.toBe(null);
      expect(LpMessage._context).toBe(testContext._context);
      // clean up
      LpMessage._context = null;
    });
  });
});
