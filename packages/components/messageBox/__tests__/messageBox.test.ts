...
import { triggerNativeCompositeClick } from '@lemon-peel/test-utils/compositeClick';
...
describe('MessageBox', () => {
  ...
  test('autofocus', async () => {
    ...
    await rAF();
    const btnElm = document.querySelector('.lp-message-box__btns .lp-button--primary')!;
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(LpMessageBox._context).toBe(null);
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      LpMessageBox.install?.(testContext as any);
      expect(LpMessageBox._context).not.toBe(null);
      expect(LpMessageBox._context).toBe(testContext._context);
      // clean up
      LpMessageBox._context = null;
    });
  });
  ...
  describe('append to', () => {
    it('should append to body if parameter is not provided', () => {
      ...
      const msgbox = document.querySelector(`body > ${selector}`) as HTMLElement;
      expect(msgbox).toBeDefined();
    });
    ...
    it('should append to HtmlElement provided', () => {
      ...
      const msgbox = htmlElement.querySelector(selector) as HTMLElement;
      expect(msgbox).toBeDefined();
    });
