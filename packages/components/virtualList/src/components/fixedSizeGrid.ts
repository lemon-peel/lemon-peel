import { isNumber, throwError } from '@lemon-peel/utils';
...
const SCOPE = 'LpFixedSizeGrid';

const FixedSizeGrid = createGrid({
  name: 'LpFixedSizeGrid',
  ...
  getColumnOffset: (
    ...
    if (alignment === 'smart') {
      alignment = scrollLeft >= minOffset - width && scrollLeft <= maxOffset + width ? AUTO_ALIGNMENT : CENTERED_ALIGNMENT;
    }
  ...
  getRowStopIndexForStartIndex: (
    ...
    const numberVisibleRows = Math.ceil(
    ...
    return Math.max(
      ...
      Math.min(
        totalRow - 1,
        startIndex + numberVisibleRows - 1, // -1 is because stop index is inclusive
      ),
    );
  },
  /**
