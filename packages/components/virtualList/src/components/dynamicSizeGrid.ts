...
import { isNumber, isUndefined, throwError } from '@lemon-peel/utils';

import { AUTO_ALIGNMENT, CENTERED_ALIGNMENT, DEFAULT_DYNAMIC_LIST_ITEM_SIZE, END_ALIGNMENT, SMART_ALIGNMENT, START_ALIGNMENT } from '../defaults';
import createGrid from '../builders/buildGrid';

import type { GridInstance } from '../builders/buildGrid';
import type { VirtualizedGridProps } from '../props';
import type { Alignment, GridCache, ItemSize, ListItem } from '../types';

const { max, min, floor } = Math;
const SCOPE = 'LpDynamicSizeGrid';
...
const ACCESS_LAST_VISITED_KEY_MAP: Record<string, keyof GridCache> = {
...
const getItemFromCache = (
  ...
  const [cachedItems, sizer, lastVisited] = [
    gridCache[type],
    props[ACCESS_SIZER_KEY_MAP[type] as keyof Props],
    gridCache[ACCESS_LAST_VISITED_KEY_MAP[type] as keyof GridCache],
  ...
  if (index > lastVisited) {
    ...
    gridCache[ACCESS_LAST_VISITED_KEY_MAP[type] as 'lastVisitedRowIndex'] = index;
  }
...
const es = (
  ...
  while (
    index < total &&
    getItemFromCache(props, index, gridCache, type).offset < offset
  ) {
    index += exponent;
    exponent *= 2;
  }

  return bs(props, gridCache, floor(index / 2), min(index, total - 1), offset, type);
};
...
const getOffset = (
  ...
  ] as [number, (props_: Props, cache: GridCache) => number];
  ...
  if (alignment === SMART_ALIGNMENT) {
    alignment = scrollOffset >= minOffset - size && scrollOffset <= maxOffset + size ? AUTO_ALIGNMENT : CENTERED_ALIGNMENT;
  }
...
const DynamicSizeGrid = createGrid({
  name: 'LpDynamicSizeGrid',
  getColumnPosition: (props, index, cache) => {
    const item = getItemFromCache(props, index, cache, 'column');
    return [item.size, item.offset];
  },

  getRowPosition: (props, index, cache) => {
    const item = getItemFromCache(props, index, cache, 'row');
    return [item.size, item.offset];
  },
...
export type ResetAfterIndex = (index: number, forceUpdate: boolean) => void;
