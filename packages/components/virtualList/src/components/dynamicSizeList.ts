...
const getEstimatedTotalSize = (
  ...
  const numberUnmeasuredItems = total - lastVisitedIndex - 1;
  const totalSizeOfUnmeasuredItems = numberUnmeasuredItems * estimatedItemSize;
  return totalSizeOfMeasuredItems + totalSizeOfUnmeasuredItems;
};

const DynamicSizeList = createList({
  name: 'LpDynamicSizeList',
  ...
  getOffset: (props, index, alignment, scrollOffset, listCache) => {
    ...
    if (alignment === SMART_ALIGNMENT) {
      alignment = scrollOffset >= minOffset - size &&
        scrollOffset <= maxOffset + size ? AUTO_ALIGNMENT : CENTERED_ALIGNMENT;
    }
  ...
  validateProps: ({ itemSize }) => {
    if (process.env.NODE_ENV !== 'production' && typeof itemSize !== 'function') {
      throwError(
        SCOPE,
        `
