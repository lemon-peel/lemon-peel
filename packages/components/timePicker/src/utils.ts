...
import { isArray, isDate, isEmpty } from '@lemon-peel/utils';
...
export const extractDateFormat = (format: string) => {
  return format
    .replaceAll(/\W?m{1,2}|\W?ZZ/g, '')
    .replaceAll(/\W?h{1,2}|\W?s{1,3}|\W?a/gi, '')
    .trim();
};

export const extractTimeFormat = (format: string) => {
  return format
    .replaceAll(/\W?D{1,2}|\W?Do|\W?d{1,4}|\W?M{1,4}|\W?Y{2,4}/g, '')
    .trim();
};
...
export const makeList = (total: number, method?: () => number[]) => {
  const array: boolean[] = [];
  const disabledArray = method?.();
  for (let index = 0; index < total; index++) {
    array.push(disabledArray?.includes(index) ?? false);
  }
  return array;
};
