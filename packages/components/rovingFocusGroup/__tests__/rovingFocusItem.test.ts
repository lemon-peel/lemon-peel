...
import { composeRefs } from '@lemon-peel/utils';
import { EVENT_CODE } from '@lemon-peel/constants';
import { ROVING_FOCUS_COLLECTION_INJECTION_KEY, ROVING_FOCUS_ITEM_COLLECTION_INJECTION_KEY } from '../src/rovingFocusGroup';
import { ROVING_FOCUS_GROUP_INJECTION_KEY, ROVING_FOCUS_GROUP_ITEM_INJECTION_KEY } from '../src/tokens';
import LpRovingFocusItem from '../src/RovingFocusItem.vue';
...
describe('<LpRovingFocusItem />', () => {
  ...
  const createComponent = (props = {}) =>
    mount(
      {
        template: `<div>
        <lp-roving-focus-item v-bind="$attrs">
          <focus-item />
  ...
  const findItems = () => wrapper.findAllComponents(LpRovingFocusItem);
