...
function useTree<T extends object>(watcherData: WatcherPropsData<T>) {
  ...
  const normalizedLazyNode = computed(() => {
    ...
    keys.forEach(key => {
      if (lazyTreeNodeMap.value[key].length > 0) {
        const item = { children: [] };
    ...
    if (keys.length > 0) {
      ...
      const getExpanded = (oldValue, key) => {
        if (ifChangeExpandRowKeys) {
          return expandRowKeys.value ? ifExpandAll || expandRowKeys.value.includes(key) : !!(ifExpandAll || oldValue?.expanded);
        } else {
          const included =
            ifExpandAll ||
