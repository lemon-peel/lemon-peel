...
export default defineComponent({
  ...
  setup(props, { emit, slots, expose }) {
    ...
    const updateActiveIndex = (value: string) => {
      ...
      activeIndex.value = item ? item.index : value;
    };
    ...
    const debounce = (function_: () => void, wait = 33.34) => {
    ...
    return () => {
      ...
      const ulStyle = useMenuCssVariable(props, 0);
      ...
      if (props.collapseTransition && props.mode === 'vertical') {
        return h(LpMenuCollapseTransition, () => vMenu);
      }
