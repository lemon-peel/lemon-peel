...
LpMessageBox({}, appContext);
// or if you are using variants
LpMessageBox.alert('Hello world!', 'Title', {}, appContext);
```
...
The corresponding methods are: `LpMessageBox`, `LpMessageBox.alert`, `LpMessageBox.confirm` and `LpMessageBox.prompt`. The parameters are the same as above.
